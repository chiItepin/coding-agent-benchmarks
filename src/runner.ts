...
program
  ...
  .action(async (options) => {
    try {
      // Load configuration
      const { config, scenarios } = await loadConfig(options.workspaceRoot || process.cwd());
      ...
      const filteredScenarios = evaluator.filterScenarios(scenarios, {
        ...
        tags: options.tag ? options.tag.split(',').map((t: string) => t.trim()) : undefined,
      });
  ...
  .action(async (options) => {
    try {
      ...
      if (options.tag) {
        ...
        filtered = filtered.filter(s => tags.some((tag: string) => s.tags.includes(tag)));
      }
